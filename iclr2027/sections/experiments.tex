...
For reviewer controls, we train additional harder2000 adapters with the same model and optimizer settings. The normalization control divides the rejected-side masked log-likelihood by active mask mass. The uniform-downweight control assigns every rejected token weight $0.25$. Localization-noise controls alter the first-error index before constructing masks, using one-step shifts and a fixed-seed random corruption of 25\% of examples.
