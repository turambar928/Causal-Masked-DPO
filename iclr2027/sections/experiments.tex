...
We evaluate the masking family through three questions. First, does vanilla DPO degrade reasoning behavior when rejected traces contain correct prefixes? Second, does masking the pre-error prefix recover performance? Third, does assigning a decayed penalty to post-error suffix steps improve over simpler first-error-only or prefix-masked alternatives? The first two questions are the core causal-masking claims; the third is the specific additional hypothesis behind \cmdpo{}.
