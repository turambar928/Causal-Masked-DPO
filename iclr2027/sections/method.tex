...
Write the DPO logit as $\Delta_\theta = \beta\left[(r_\theta(\yw\mid x)-r_{\mathrm{ref}}(\yw\mid x))-(r_\theta^w(\yl\mid x)-r_{\mathrm{ref}}^w(\yl\mid x))\right]$. Then the gradient contribution of any rejected token $j$ is proportional to $w_j$ times the usual DPO factor. For a protected prefix token, $w_j=0$, so that token contributes no rejected-side negative gradient at all. For the first error, $w_j=1$, so it receives the full rejected-side pressure. For suffix tokens, the decayed weights interpolate smoothly between these extremes. This makes the causal meaning of the mask explicit: we are not changing the preferred response, only choosing which rejected tokens should count as evidence against the policy.
