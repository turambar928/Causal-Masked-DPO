...
def weights_for_variant(num_steps: int, first_error: int, variant: str, gamma: float) -> list[float]:
    if variant == "vanilla":
...
def main() -> None:
    ...
    for variant in ["vanilla", "prefix_masked", "first_error_only", "cmdpo"]:
        variant_rows = []
        for row in rows:
            item = dict(row)
            steps = item["rejected_steps"]
            first_error = int(item["first_error_step"])
            item["step_weights"] = weights_for_variant(len(steps), first_error, variant, args.gamma)
